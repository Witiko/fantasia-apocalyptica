...
\begingroup
\small
% TODO: Přesuň do plovoucí ukázky kódu
\begin{minted}{python}
...
Jeden řádek výstupního souboru \texttt{časování-ilustrací.txt} je např. \texttt{194.png 1119}. To znamená, že obrázek \texttt{ilustrace/194.png} s ilustrací verše 21:08 trvá 1\,119 snímků videa, což je při frekvenci 50 snímků za vteřinu přibližně 22 vteřin.
...
\item \textls[-10]{Po obrázcích dlouhých 2–5 vteřin následuje krátká animace přechodu (520\,ms).}
\item Po obrázcích delších než 5 vteřin následuje dlouhá animace přechodu (1\,s).
\end{itemize}
Např. po obrázku \verb|ilustrace/194.png| následuje dlouhá animace přechodu.

Dále jsem pro každý obrázek nástrojem FFmpeg vygeneroval video, jehož délka odpovídala době trvání obrázku po odečtení doby trvání animace. Např. video pro obrázek \verb|ilustrace/194.png| trvá 1\,119 snímků videa po odečtení jedné vteřiny pro dlouhou animaci přechodu:
\begin{verbatim}
...
>          výstupní-video.mp4
\end{Verbatim}
\footnotetext{Proměnná \texttt{\$N} značí snímek, po kterém se obdélník změní na poloprůhledný, a liší se pro jednotlivá videa not. Např. ve videu pro 21. kapitolu jsem nastavil \texttt{N=2700}. Při frekvenci 50 snímků za vteřinu tedy dojde ke změně po 54 vteřinách, kdy již Tomáš posunul noty tak, že horní šedivý okraj není vidět.}
...
>        -map '[kolaz]' -t 01:43:25 -r 50 \
>        -c:v libx265 -crf 17 -preset veryslow -pix_fmt yuv420p \
>        -c:a aac -b:a 384k -ar 44100 výstupní-video.mp4
\end{verbatim}
Následně jsem k výsledku připojil videozáznam úvodního slova a závěru~\cite{ffmpeg-concat}. Obrázek~\ref{fig:montage} ukazuje výsledné video.

\section{Závěr}
\label{sec:zaver}
TODO: Popsat promítání videozáznamu 18. prosince 2019 včetně popisu plakátu a nahánění studentů z haly za zvuku PC speakerů. \url{https://gitlab.fi.muni.cz/unix/beeplay/-/commits/master/?ref_type=HEADS}
